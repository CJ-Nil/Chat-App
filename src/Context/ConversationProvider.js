...
export function ConversationsProvider({ id,children }) {
    ...
    function sendMessage(recipients,text){
        ...
        const brodcastid='*'
        if(recipients.includes(brodcastid)){
            contacts.forEach(contact => {
                socket.emit('send-message',{recipients:[contact.id],text,d})
            })
        } else {
            socket.emit('send-message',{recipients,text,d})
        }
        addMessageToConversation({recipients,text,sender: id,date:d})
    }
    const formatedConversations = conversations.map((conversation,index) => {
        ...
        const messages = conversation.messages.map(message => {
            ...
            return {...message,senderName:name,fromMe,date:date}
        })
